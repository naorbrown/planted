...
test.describe('Content', () => {
  test('home page has hero section', async ({ page }) => {
    await page.goto('/');
    await expect(page.locator('h1')).toContainText('every space');
  });
  ...
  test('plant detail has care card on desktop', async ({ page }) => {
    ...
    await expect(page.getByText('Care Guide')).toBeVisible();
  });
  ...
  test('guide detail page loads', async ({ page }) => {
    await page.goto('/guides/beginners-guide-to-indoor-plants/');
